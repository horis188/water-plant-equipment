...
async function loadPermissionCodes(user: { id: number; role_id?: number; role: string }) {
  if (!user.role_id) {
  ...
  try {
    const res = await fetch(`/api/admin/roles/${user.role_id}/permissions`, {
...
const handleLogin = async (user: { id: number; username: string; name: string; role: string; role_id?: number; team?: string; member_name?: string; avatar?: string }) => {
  ...
  await loadPermissionCodes({ id: user.id, role_id: user.role_id, role: user.role })
